...
class EightPuzzleState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        x = -1
        y = -1
        down = [[i for i in row] for row in self.state]
        left = [[i for i in row] for row in self.state]
        up = [[i for i in row] for row in self.state]
        right = [[i for i in row] for row in self.state]
        ...
        if x != 2:
            down[x][y] = down[x+1][y]
            down[x+1][y] = 0
            new_state = EightPuzzleState(down, self.goal, dist_from_start=self.dist_from_start + 1, use_heuristic=self.use_heuristic, zero_loc=5)
            nbr_states.append(new_state)

        if y != 0:
            left[x][y] = left[x][y-1]
            left[x][y-1] = 0
            new_state = EightPuzzleState(left, self.goal, dist_from_start=self.dist_from_start + 1, use_heuristic=self.use_heuristic, zero_loc=5)
            nbr_states.append(new_state)

        if x != 0:
            up[x][y] = up[x-1][y]
            up[x-1][y] = 0
            new_state = EightPuzzleState(up, self.goal, dist_from_start=self.dist_from_start + 1, use_heuristic=self.use_heuristic, zero_loc=5)
            nbr_states.append(new_state)

        if y != 2:
            right[x][y] = right[x][y+1]
            right[x][y+1] = 0
            new_state = EightPuzzleState(right, self.goal, dist_from_start=self.dist_from_start + 1, use_heuristic=self.use_heuristic, zero_loc=5)
            nbr_states.append(new_state)
    ...
    def compute_heuristic(self):
        ...
        for i in range(9):
            ...
            if cur != 0:
                total += manhattan([x,y],[cur//3,cur%3])
